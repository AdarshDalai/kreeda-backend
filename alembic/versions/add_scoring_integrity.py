...
def upgrade():
    ...
    scorer_role_enum = postgresql.ENUM('team_a_scorer', 'team_b_scorer', 'umpire', 'referee', name='scorerrole')
    scorer_role_enum.create(op.get_bind())
    
    # Create match_scorers table
    op.create_table('match_scorers',
        ...
        sa.Column('appointed_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.ForeignKeyConstraint(['appointed_by_id'], ['users.id'], ),
    ...
    op.create_table('ball_score_entries',
        ...
        sa.Column('batsman_non_striker_id', postgresql.UUID(as_uuid=True), nullable=False),
        ...
        sa.Column('entry_timestamp', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('is_verified', sa.Boolean(), nullable=True),
        sa.Column('verification_status', sa.String(length=20), nullable=True),
        sa.ForeignKeyConstraint(['batsman_non_striker_id'], ['users.id'], ),
        sa.ForeignKeyConstraint(['batsman_striker_id'], ['users.id'], ),
        sa.ForeignKeyConstraint(['bowler_id'], ['users.id'], ),
        sa.ForeignKeyConstraint(['dismissed_player_id'], ['users.id'], ),
        sa.ForeignKeyConstraint(['match_id'], ['cricket_matches.id'], ),
        sa.ForeignKeyConstraint(['scorer_id'], ['users.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index('ix_ball_score_entries_match_id', 'ball_score_entries', ['match_id'], unique=False)

    # Create ball_verifications table
    op.create_table('ball_verifications',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('match_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('innings', sa.Integer(), nullable=False),
        sa.Column('over_number', sa.Integer(), nullable=False),
        ...
        sa.ForeignKeyConstraint(['final_entry_id'], ['ball_score_entries.id'], ),
        sa.ForeignKeyConstraint(['match_id'], ['cricket_matches.id'], ),
    ...
    op.create_table('scoring_audit_logs',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('match_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), nullable=False),
    ...
    op.create_index('ix_scoring_audit_logs_match_id', 'scoring_audit_logs', ['match_id'], unique=False)

    # Create match_integrity_checks table
    op.create_table('match_integrity_checks',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('match_id', postgresql.UUID(as_uuid=True), nullable=False),
