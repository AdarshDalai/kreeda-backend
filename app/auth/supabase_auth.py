...
# Auth functions
def sign_up_with_email_password(email: str, password: str, user_metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
        ...
        if settings.environment in ["development", "testing"]:
            try:
                ...
                if admin_response and hasattr(admin_response, 'user') and admin_response.user:
                    logger.info(f"User created with admin client (auto-confirmed): {email}")
                ...
                logger.warning(f"Admin user creation failed for {email}, falling back to regular signup: {str(admin_error)}")
...
def sign_in_with_oauth(provider: str, redirect_to: Optional[str] = None) -> Dict[str, Any]:
    """
...
def reset_password_email(email: str, redirect_to: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Password reset method not available - would send reset to: {email}")
            response = True
