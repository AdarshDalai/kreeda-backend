...
from app.schemas.cricket import (
    CricketMatchCreate, BallRecord, BallResponse, 
    MatchResponseBasic, ScorerAssignment
)
...
async def assign_match_scorers(
    ...
    try:
        ...
        if str(match.created_by_id) != str(current_user.id):
            raise HTTPException(status_code=403, detail="Only match creator can assign scorers")
...
@router.post("/matches/{match_id}/disputes/{innings}/{over_number}/{ball_number}/resolve")
async def resolve_scoring_dispute(
    ...
    try:
        ...
        user_roles = [s["role"] for s in scoring_status["scorers"] if s["user_id"] == str(current_user.id)]
        
        if "umpire" not in user_roles and "referee" not in user_roles:
            raise HTTPException(status_code=403, detail="Only umpires/referees can resolve disputes")
