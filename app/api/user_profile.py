...
from app.schemas.user_profile import (
    ...
    UserProfileResponse, UserDashboardStats, UserActivityLog
)
...
async def get_profile(
    ...
    try:
        profile = await UserProfileService.get_complete_profile(db, str(current_user.id))
...
async def get_profile_completion(
    ...
    try:
        ...
        if not sports_profile or not (isinstance(sports_profile, dict) and sports_profile.get('favorite_sports')):
        ...
        return {
            ...
            "completed_sections": {
                "basic_info": bool(profile.full_name and profile.email and profile.username),
                "profile_details": bool(profile.bio and profile.location),
                "avatar": bool(profile.avatar_url),
                "sports_profile": bool(sports_profile and isinstance(sports_profile, dict) and sports_profile.get('favorite_sports')),
        ...
        logger.error(f"Error getting profile completion for user {current_user.id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
