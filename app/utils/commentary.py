...
class CricketCommentaryGenerator:
    ...
    def generate_over_summary(self, over_balls: List[CricketBall], over_runs: int) -> str:
        """Generate end-of-over summary"""
        over_number = getattr(over_balls[0], 'over_number', 1) if over_balls else 1
        
        wickets_in_over = sum(1 for ball in over_balls if getattr(ball, 'is_wicket', False))
        boundaries_in_over = sum(1 for ball in over_balls if getattr(ball, 'is_boundary', False))
