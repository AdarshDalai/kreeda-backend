...
class ScoringIntegrityService:
    ...
    async def record_ball_entry(
        ...
        try:
            ...
            return {
                ...
                "consensus_reached": verification_result.get(
    ...
    async def _check_ball_verification(
        self, match_id: str, innings: int, over_number: int, ball_number: int
    ) -> Dict:
        ...
        try:
            ...
            return {
                "status": (
                    "verified" if consensus_data["consensus_reached"] else "disputed"
                ),
    ...
    async def _update_ball_verification(
        ...
        try:
            ...
            setattr(
                verification, "consensus_reached", consensus_data["consensus_reached"]
            )
            setattr(
                verification, "has_dispute", not consensus_data["consensus_reached"]
            )

            if consensus_data["consensus_reached"]:
                setattr(
                    verification, "final_entry_id", consensus_data["consensus_entry"].id
                )
    ...
    async def _create_official_ball_record(
        ...
        try:
            ...
            if existing_ball is None:
                ...
                logger.info(
                    f"Created official ball record for match {match_id}, over {verification.over_number}.{verification.ball_number}"
                )
    ...
    async def get_match_scoring_status(self, match_id: str) -> Dict:
        ...
        try:
            ...
            scorers_query = select(MatchScorer).where(
                and_(MatchScorer.match_id == match_id, MatchScorer.is_active == True)
            )
            ...
            return {
                ...
                "disputes": [
                    ...
                    if getattr(v, "has_dispute", False)
                ],
            }
