    ...
    async def record_ball(self, match_id: str, ball_data: BallRecord) -> tuple:
        ...
        try:
            ...
            async with self.db.begin():
                ...
                if current_innings_val == 1:
                    await self.db.execute(
                        ...
                        .values(
                            ...
                            team_a_wickets=CricketMatch.team_a_wickets
                            + wicket_increment,
                    ...
                    await self.db.execute(
                        ...
                        .values(
                            ...
                            team_b_wickets=CricketMatch.team_b_wickets
                            + wicket_increment,
                ...
                match_context = {
                    "current_score": (
                        updated_match.team_a_score
                        if updated_match.current_innings == 1
                        else updated_match.team_b_score
                    ),
                    "current_wickets": (
                        updated_match.team_a_wickets
                        if updated_match.current_innings == 1
                        else updated_match.team_b_wickets
                    ),
                    "required_run_rate": getattr(updated_match, "required_run_rate", 0),
                    "current_run_rate": getattr(updated_match, "current_run_rate", 0),
                    "overs_remaining": updated_match.overs_per_innings
                    - (
                        updated_match.team_a_overs
                        if updated_match.current_innings == 1
                        else updated_match.team_b_overs or 0
                    ),
                }

                commentary = commentary_generator.generate_ball_commentary(
                    ball, match_context
                )
                ...
                await websocket_manager.broadcast_ball_update(
                    match_id, ball_data, scorecard
                )
                ...
                if getattr(ball, "is_wicket", False):
                    ...
                    await websocket_manager.broadcast_wicket_alert(
                        match_id, wicket_data
                    )
                ...
                if getattr(ball, "is_boundary", False):
                    ...
                    await websocket_manager.broadcast_boundary_alert(
                        match_id, boundary_data
                    )
