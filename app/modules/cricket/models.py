...
class CricketTeam(BaseModel):
    ...
    captain_id: Mapped[Optional[int]] = mapped_column()  # No FK to avoid circular dependency
    
    # Relationships
    players = relationship("Player", foreign_keys="Player.team_id", back_populates="team")
    # Note: captain relationship handled via application logic to avoid circular dependency
...
class Player(BaseModel):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    name: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    team_id: Mapped[Optional[int]] = mapped_column(ForeignKey("cricket_teams.id"))
    ...
    # Relationships
    team = relationship("CricketTeam", foreign_keys="Player.team_id", back_populates="players")
...
class Match(BaseModel):
    ...
    # Teams
    home_team_id: Mapped[int] = mapped_column(ForeignKey("cricket_teams.id"), nullable=False)
    away_team_id: Mapped[int] = mapped_column(ForeignKey("cricket_teams.id"), nullable=False) 
    ...
    # Toss details
    toss_winner_id: Mapped[Optional[int]] = mapped_column(ForeignKey("cricket_teams.id"))
    toss_decision: Mapped[Optional[str]] = mapped_column(String(10))  # BAT, BOWL
    
    # Match result
    winner_id: Mapped[Optional[int]] = mapped_column(ForeignKey("cricket_teams.id"))
    ...
    # Relationships
    home_team = relationship("CricketTeam", foreign_keys=[home_team_id])
    away_team = relationship("CricketTeam", foreign_keys=[away_team_id])
    toss_winner = relationship("CricketTeam", foreign_keys=[toss_winner_id])
    winner = relationship("Team", foreign_keys=[winner_id])
    innings = relationship("Innings", back_populates="match", cascade="all, delete-orphan")

    # Constraints
    __table_args__ = (
        CheckConstraint('home_team_id != away_team_id', name='cricket_different_teams'),
        CheckConstraint('overs_per_innings > 0', name='cricket_positive_overs'),
        CheckConstraint('balls_per_over > 0', name='cricket_positive_balls_per_over'),
        CheckConstraint('max_wickets > 0', name='cricket_positive_max_wickets'),
    )
...
class Innings(BaseModel):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    match_id: Mapped[int] = mapped_column(ForeignKey("cricket_matches.id"), nullable=False)
    innings_number: Mapped[int] = mapped_column(Integer, nullable=False)  # 1, 2, 3, 4 for Test matches
    batting_team_id: Mapped[int] = mapped_column(ForeignKey("cricket_teams.id"), nullable=False)
    bowling_team_id: Mapped[int] = mapped_column(ForeignKey("cricket_teams.id"), nullable=False)
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint('match_id', 'innings_number', name='unique_cricket_innings_per_match'),
        CheckConstraint('total_runs >= 0', name='cricket_non_negative_runs'),
        CheckConstraint('total_wickets >= 0', name='cricket_non_negative_wickets'),
        CheckConstraint('total_overs >= 0', name='cricket_non_negative_overs'),
        CheckConstraint('innings_number > 0', name='cricket_positive_innings_number'),
    )
...
class Over(BaseModel):
    """Cricket over model"""
    __tablename__ = "cricket_overs"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    innings_id: Mapped[int] = mapped_column(ForeignKey("cricket_innings.id"), nullable=False)
    over_number: Mapped[int] = mapped_column(Integer, nullable=False)
    bowler_id: Mapped[int] = mapped_column(ForeignKey("cricket_players.id"), nullable=False)
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint('innings_id', 'over_number', name='cricket_unique_over_per_innings'),
        CheckConstraint('over_number > 0', name='cricket_positive_over_number'),
        ...
        CheckConstraint('balls_bowled >= 0', name='cricket_non_negative_balls_bowled'),
        CheckConstraint('legal_balls >= 0', name='cricket_non_negative_legal_balls'),
    )
...
class Ball(BaseModel):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    over_id: Mapped[int] = mapped_column(ForeignKey("cricket_overs.id"), nullable=False)
    ball_number: Mapped[int] = mapped_column(Integer, nullable=False)  # 1-6 (or more for extras)
    
    # Ball details
    bowler_id: Mapped[int] = mapped_column(ForeignKey("cricket_players.id"), nullable=False)
    batsman_id: Mapped[int] = mapped_column(ForeignKey("cricket_players.id"), nullable=False)
    non_striker_id: Mapped[int] = mapped_column(ForeignKey("cricket_players.id"), nullable=False)
    ...
    dismissed_player_id: Mapped[Optional[int]] = mapped_column(ForeignKey("cricket_players.id"))
    fielder_id: Mapped[Optional[int]] = mapped_column(ForeignKey("cricket_players.id"))  # For catches, run-outs
    ...
    __table_args__ = (
        ...
        CheckConstraint('extra_runs >= 0', name='cricket_non_negative_extra_runs'),
        CheckConstraint('ball_number > 0', name='cricket_positive_ball_number'),
        CheckConstraint(
...
class BattingPerformance(BaseModel):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    innings_id: Mapped[int] = mapped_column(ForeignKey("cricket_innings.id"), nullable=False)
    player_id: Mapped[int] = mapped_column(ForeignKey("cricket_players.id"), nullable=False)
    ...
    dismissal_type: Mapped[Optional[DismissalType]] = mapped_column(String(30))
    bowler_id: Mapped[Optional[int]] = mapped_column(ForeignKey("cricket_players.id"))  # Bowler who took wicket
    fielder_id: Mapped[Optional[int]] = mapped_column(ForeignKey("cricket_players.id"))  # Fielder involved
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint('innings_id', 'player_id', name='cricket_unique_batting_performance'),
        UniqueConstraint('innings_id', 'batting_position', name='cricket_unique_batting_position'),
        CheckConstraint('runs_scored >= 0', name='non_negative_runs_scored'),
